...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        /* ✨ จุดที่ 4 แก้ไข: ปรับหน้ากระดาษว่างตอนปริ้นให้ออกไป ✨ */
        @media print {
            .no-print { display: none !important; }
            body, html { height: auto !important; overflow: visible !important; display: block !important; background: white !important; font-size: 14px; color: black !important; }
            ...
            .a4-container { 
                ...
                page-break-after: always !important; /* บังคับขึ้นหน้าใหม่เสมอ */
                break-after: page !important;
                page-break-inside: avoid !important;
            }
            /* ยกเว้นหน้าสุดท้าย ห้ามมีการบังคับขึ้นหน้าใหม่ จะได้ไม่มีกระดาษเปล่าติดมา */
...
<body class="bg-slate-50 text-slate-800 dark:bg-slate-800 dark:text-slate-100 min-h-screen flex flex-col">
        ...
        <div class="max-w-7xl mx-auto flex flex-col gap-4">
            ...
            <div class="flex flex-col lg:flex-row justify-between items-center gap-4">
                ...
                <div class="flex items-center space-x-4 w-full lg:w-auto justify-center lg:justify-start shrink-0">
                    <a href="dashboard.php?tab=reports" class="bg-violet-50 hover:bg-violet-100 text-violet-700 border border-violet-200 dark:bg-violet-600/20 dark:hover:bg-violet-500/30 dark:border-violet-600/50 dark:text-violet-300 px-4 py-1.5 rounded-full text-xs font-bold transition-all shadow-sm flex items-center">
                        <i class="fas fa-arrow-left mr-2"></i> Dashboard
                    </a>
                    <h1 class="font-extrabold text-sm border-l-2 border-slate-200 dark:border-slate-700 pl-4 text-slate-800 dark:text-slate-200 tracking-wide hidden sm:block">ระบบพิมพ์เอกสารรายงาน</h1>
                </div>
                ...
                <div class="flex flex-wrap items-center justify-center lg:justify-end gap-2.5 w-full lg:w-auto pb-0.5">
                    <form method="GET" action="print_report.php" class="flex flex-wrap items-center gap-2.5 bg-slate-50 dark:bg-slate-800 p-1.5 px-2.5 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-inner">
                        ...
                        <div class="relative w-full md:w-60" id="techDropdownContainer">
                            <div class="flex items-center w-full bg-white dark:bg-slate-700 text-slate-700 dark:text-slate-100 font-bold text-xs rounded-full border border-slate-200 dark:border-slate-600 shadow-sm focus-within:ring-2 focus-within:ring-indigo-400 transition-colors cursor-text overflow-hidden" onclick="toggleDropdown('techDropdownList', event)">
                                ...
                                <button type="button" class="pr-3 pl-1 text-slate-400 dark:text-slate-300 focus:outline-none flex items-center justify-center pointer-events-none">
                                    <i class="fas fa-caret-down text-sm"></i>
                                </button>
                            </div>
                            
                            <div id="techDropdownList" class="absolute z-50 w-full md:w-72 mt-1 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-xl shadow-xl max-h-80 overflow-y-auto hidden flex-col py-2 custom-scrollbar right-0 md:right-auto md:left-0">
                                
                                <div class="tech-dropdown-item px-3 py-1.5 mx-2 rounded-lg text-xs font-bold text-slate-600 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 cursor-pointer transition-colors flex items-center" data-value="all" data-search="รวมทุกฝ่ายงานทั้งหมด" onmousedown="selectOption('tech', 'all', 'รวมทุกฝ่ายงาน (ทั้งหมด)')">
                                    <div class="w-5 h-5 rounded-full bg-slate-100 dark:bg-slate-600 flex items-center justify-center mr-2.5 text-slate-400 dark:text-slate-300">
                                ...
                                foreach($grouped_techs as $dept => $techs) {
                                    ...
                                    echo "<div class='flex justify-between items-center px-4 py-1.5 mt-1.5 mb-0.5 bg-blue-50/50 dark:bg-slate-900 border-y border-slate-100 dark:border-slate-800 dropdown-dept-header' data-dept=\"".htmlspecialchars($dept)."\">
                                            <span class='text-[11px] font-extrabold text-indigo-600 dark:text-indigo-400 tracking-wide'>{$dept}</span>
                                            <span class='text-[9px] font-bold text-slate-500 dark:text-slate-400 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-1.5 py-0.5 rounded flex items-center'>
                                                <i class='fas fa-user-friends mr-1'></i> {$tech_count} คน
                                            </span>
                                          </div>";

                                    foreach($techs as $t_name) {
                                        $searchStr = preg_replace('/\s+/', '', strtolower($t_name . $dept));
                                        echo "<div class='tech-dropdown-item px-3 py-1.5 mx-2 mb-0.5 rounded-lg text-xs text-slate-700 dark:text-slate-200 font-bold hover:bg-indigo-50 dark:hover:bg-slate-700 hover:text-indigo-600 dark:hover:text-indigo-300 cursor-pointer flex items-center transition-all group' data-value=\"".htmlspecialchars($t_name)."\" data-display=\"".htmlspecialchars($t_name)."\" data-search=\"{$searchStr}\" data-dept=\"".htmlspecialchars($dept)."\" onmousedown=\"selectOption('tech', '".htmlspecialchars($t_name, ENT_QUOTES)."', '".htmlspecialchars($t_name, ENT_QUOTES)."')\">
                                                <div class='w-5 h-5 rounded-full bg-slate-100 dark:bg-slate-600 flex items-center justify-center mr-2.5 text-slate-400 dark:text-slate-300'>
                        ...
                        <!-- ✨ จุดที่ 1 แก้ไข: เปลี่ยน Dropdown เดือนให้โค้งมนและใช้ฟอนต์เดียวกับระบบ คล้ายปุ่มค้นหาช่าง ✨ -->
                        <div class="relative w-28 sm:w-auto" id="monthDropdownContainer">
                            <div class="flex items-center w-full bg-white dark:bg-slate-700 text-slate-700 dark:text-slate-100 font-bold text-xs rounded-full border border-slate-200 dark:border-slate-600 shadow-sm focus-within:ring-2 focus-within:ring-indigo-400 transition-colors cursor-pointer overflow-hidden" onclick="toggleDropdown('monthDropdownList', event)">
                                <span id="monthDisplayText" class="px-3 py-2 w-full text-left truncate"><?= $thai_months[$selected_month] ?></span>
                                <button type="button" class="pr-3 pl-1 text-slate-400 dark:text-slate-300 focus:outline-none flex items-center justify-center pointer-events-none">
                                    <i class="fas fa-caret-down text-sm"></i>
                                </button>
                            </div>
                            
                            <div id="monthDropdownList" class="absolute z-50 w-32 mt-1 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-xl shadow-xl max-h-60 overflow-y-auto hidden flex-col py-2 custom-scrollbar right-0 sm:left-0 sm:right-auto">
                                <?php for($m=1; $m<=12; $m++): ?>
                                    <div class="px-3 py-1.5 mx-2 mb-0.5 rounded-lg text-xs font-bold cursor-pointer transition-colors text-center <?= ($selected_month === $m) ? 'bg-indigo-50 dark:bg-slate-700 text-indigo-600 dark:text-indigo-400' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700' ?>" onmousedown="selectOption('month', <?= $m ?>, '<?= $thai_months[$m] ?>')">
                                        <?= $thai_months[$m] ?>
                                    </div>
                                <?php endfor; ?>
                            </div>
                            <input type="hidden" name="month" id="monthHiddenInput" value="<?= $selected_month ?>">
                        </div>

                        <!-- ✨ จุดที่ 1 แก้ไข: เปลี่ยน Dropdown ปีให้โค้งมนและใช้ฟอนต์เดียวกับระบบ คล้ายปุ่มค้นหาช่าง ✨ -->
                        <div class="relative w-24 sm:w-auto" id="yearDropdownContainer">
                            <div class="flex items-center w-full bg-white dark:bg-slate-700 text-slate-700 dark:text-slate-100 font-bold text-xs rounded-full border border-slate-200 dark:border-slate-600 shadow-sm focus-within:ring-2 focus-within:ring-indigo-400 transition-colors cursor-pointer overflow-hidden" onclick="toggleDropdown('yearDropdownList', event)">
                                <span id="yearDisplayText" class="px-3 py-2 w-full text-left truncate">พ.ศ. <?= ($selected_year + 543) ?></span>
                                <button type="button" class="pr-3 pl-1 text-slate-400 dark:text-slate-300 focus:outline-none flex items-center justify-center pointer-events-none">
                                    <i class="fas fa-caret-down text-sm"></i>
                                </button>
                            </div>
                            
                            <div id="yearDropdownList" class="absolute z-50 w-28 mt-1 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-xl shadow-xl max-h-60 overflow-y-auto hidden flex-col py-2 custom-scrollbar right-0 sm:left-0 sm:right-auto">
                                <?php foreach($available_years as $y): ?>
                                    <div class="px-3 py-1.5 mx-2 mb-0.5 rounded-lg text-xs font-bold cursor-pointer transition-colors text-center <?= ($selected_year == $y) ? 'bg-indigo-50 dark:bg-slate-700 text-indigo-600 dark:text-indigo-400' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700' ?>" onmousedown="selectOption('year', <?= $y ?>, 'พ.ศ. <?= ($y + 543) ?>')">
                                        พ.ศ. <?= ($y + 543) ?>
                                    </div>
                                <?php endforeach; ?>
                            </div>
                            <input type="hidden" name="year" id="yearHiddenInput" value="<?= $selected_year ?>">
                        </div>
                    ...
                    <button id="theme-toggle" type="button" class="w-8 h-8 rounded-full bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-500 dark:text-amber-400 shadow-sm hover:bg-slate-50 dark:hover:bg-slate-600 transition-all flex items-center justify-center shrink-0">
                        <i id="theme-toggle-icon" class="fas fa-moon"></i>
                    </button>
                </div>
            </div>

            <!-- บรรทัดที่ 2: ซ้าย (ตาราง/ข้อความ + พิมพ์) | ขวา (สวิตช์ลายเซ็น) -->
            <div class="flex flex-col md:flex-row justify-between items-center gap-4 mt-2 md:mt-0">
                
                <div class="flex flex-wrap items-center justify-center sm:justify-start gap-2 w-full sm:w-auto">
                    <!-- ✨ จุดที่ 3 แก้ไข: กรอบของปุ่มที่ยังไม่ได้กดในโหมดมืดจะมีสีเทาและเห็นชัดขึ้น ✨ -->
                    <a href="print_report.php?type=table&tech=<?php echo urlencode($selected_tech); ?>&month=<?php echo $selected_month; ?>&year=<?php echo $selected_year; ?>" 
                       class="px-4 py-1.5 rounded-full text-xs font-bold transition-all flex items-center border <?= $report_type === 'table' ? 'bg-indigo-50 text-indigo-700 border-indigo-200 dark:bg-indigo-600/20 dark:border-indigo-500 dark:text-indigo-300 shadow-sm' : 'bg-white dark:bg-slate-700 text-slate-500 dark:text-slate-300 border-slate-200 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-600 shadow-sm' ?>">
                    ...
                    <a href="print_report.php?type=memo&tech=<?php echo urlencode($selected_tech); ?>&month=<?php echo $selected_month; ?>&year=<?php echo $selected_year; ?>" 
                       class="px-4 py-1.5 rounded-full text-xs font-bold transition-all flex items-center border <?= $report_type === 'memo' ? 'bg-indigo-50 text-indigo-700 border-indigo-200 dark:bg-indigo-600/20 dark:border-indigo-500 dark:text-indigo-300 shadow-sm' : 'bg-white dark:bg-slate-700 text-slate-500 dark:text-slate-300 border-slate-200 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-600 shadow-sm' ?>">
                        <i class="fas fa-file-alt mr-1.5"></i> บันทึกข้อความ
                    </a>
                    
                    <button type="button" onclick="window.print()" class="bg-slate-900 hover:bg-black dark:bg-rose-800 dark:hover:bg-rose-700 text-white text-xs px-5 py-1.5 rounded-full font-bold shadow-md transition-all flex items-center ml-1 border border-slate-900 dark:border-rose-800">
                ...
                <div class="flex items-center justify-center sm:justify-end w-full sm:w-auto pr-1">
                    <!-- ✨ จุดที่ 2 แก้ไข: ปุ่มลายเซ็นมีกรอบเหมือนกันและโหมดมืดเห็นชัดเจนขึ้น (ปุ่มสวิตช์อยู่ด้านหน้าข้อความ) ✨ -->
                    <label for="toggleSignature" class="flex items-center cursor-pointer bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 px-3.5 py-1.5 rounded-full hover:bg-slate-50 dark:hover:bg-slate-600 transition-colors shadow-sm group">
                        <div class="relative flex items-center">
                            <input type="checkbox" id="toggleSignature" class="sr-only peer" checked onchange="toggleSignature()">
                            <div class="w-8 h-4.5 bg-slate-300 dark:bg-slate-500 peer-focus:outline-none rounded-full peer peer-checked:bg-indigo-500 transition-colors duration-300 shadow-inner"></div>
                            <div class="absolute left-[2px] top-[2px] bg-white rounded-full h-3.5 w-3.5 transition-transform duration-300 peer-checked:translate-x-[14px] shadow-sm"></div>
                        </div>
                        <span class="ml-3 text-[11px] font-bold text-slate-600 dark:text-slate-300 group-hover:text-slate-800 dark:group-hover:text-white transition-colors">ลายเซ็นท้ายเอกสาร</span>
                    </label>
                </div>
    ...
    <!-- เพิ่มคลาส print:pb-0 print:overflow-visible เพื่อแก้ปัญหาหน้ากระดาษว่างตอนปริ้น -->
    <div class="flex-1 overflow-auto pb-10 print:pb-0 print:overflow-visible">
    ...
    <script>
        ...
        // ระบบจัดสรรหน้าต่าง Dropdown (Custom Dropdown)
        ...
        function focusTechSearch(e) {
            e.target.value = ''; 
            filterTechDropdown(); 
            toggleDropdown('techDropdownList', e, true);
        }
        ...
        function toggleDropdown(listId, e, forceOpen = false) {
            if(e) e.stopPropagation();
            
            // ปิด Dropdown ตัวอื่นๆ หากมีการกดเปิดตัวใดตัวหนึ่ง
            const lists = ['techDropdownList', 'monthDropdownList', 'yearDropdownList'];
            lists.forEach(id => {
                if (id !== listId) {
        ...
        function filterTechDropdown() {
            toggleDropdown('techDropdownList', null, true);
        ...
        function selectOption(type, val, displayText) {
            if (type === 'tech') {
                currentTechDisplay = displayText;
                document.getElementById('techSearchInput').value = displayText;
            } else {
                document.getElementById(type + 'DisplayText').innerText = displayText;
            }
            document.getElementById(type + 'HiddenInput').value = val;
            document.getElementById(type + 'DropdownList').classList.add('hidden');
            document.getElementById(type + 'DropdownList').classList.remove('flex');
        }
